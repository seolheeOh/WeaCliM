...
for inp_month in ['apr', 'oct']:
    ...
    pr = np.zeros((20, num_ens, 1, 29, 72))
    for e, ens in enumerate(range(1,num_ens+1)):
        for i, lon in enumerate(range(0, 360, 5)):
            for j, lat in enumerate(range(-70, 75, 5)):

                ipath = f'../../Outputs/ref_CNN_monthly/Main_{inp_month}_{target_month}/'
                f = Dataset(f'{ipath}/fcst_ens0{ens}_{lon}E_{lat}N.nc','r')
    ...
    pr = pr.reshape(20, num_ens, 1, 29, 72)

    oname = f'ref_cnn_monthly_{inp_month}_{target_month}_gpcp_2000-2019'
    opath = 'data/reconst/'
