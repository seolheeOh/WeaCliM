...
for target in ['JJA', 'DJF']:
    ...
    for area in ['IN', 'EA', 'NAF', 'NAM', 'SAM', 'SAF']:
        ...
        ipath = 'data/reconst/'
        fname = f'avg.dyn_{inp_month}_{target}_prcp_2000-2019.nc'

        f = Dataset(ipath+fname, 'r')
        data = f.variables['p'][:].squeeze()

        data = np.where(data<-1e8, np.nan, data)
        ...
        oname = f'avg.dyn_{inp_month}_{target}_prcp_{area}_2000-2019'
