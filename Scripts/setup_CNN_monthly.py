...
def train_model_for_grid(grid_i, grid_j, season, ens):
    ...
    ipath = '../Dataset/'
    f = Dataset(f'{ipath}/inp_monthly_{inp_month}_2000-2019.nc','r')
    ...
    predictions = np.zeros((tdim, 1))
    for i in range(tdim):
        target = str(i+2000)

        exp_path = f'../Outputs/ref_CNN_monthly/Main_{inp_month}_{season}/'
        opath = exp_path+'/'+target+'/ENS0'+str(ens)+'_'+str(lon)+'E_'+str(lat)+'N/'
        pathlib.Path(opath).mkdir(parents=True, exist_ok=True)

        model = ReferenceCNN(device, opath, 19, 20, 0.001)
