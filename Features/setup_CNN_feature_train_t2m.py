...
#    print(f"Training on grid {grid_i} and {grid_j}")
    ...
    ipath = '../Dataset/'
    f = Dataset(ipath+'inp_daily_apr_2000-2019.nc','r')
    inp[:,0] = f.variables['t2m'][:,0]
    f.close()

    # augmenation with last 10 days
...
#    print(f"Using LON: {lon}, LAT: {lat}")
    ...
    for i in range(tdim):
        ...
        exp_path = '../Outputs/ref_CNN_daily/Main_apr_JJA/'
        opath = f'{exp_path}/{target}/ENS0{ens}_{lon}E_{lat}N/'

        model = ExtractFeature(device=device, opath=opath)
    ...
    save_path = '../Outputs/ref_CNN_daily_feature/train_t2m/'
...
#    print(data.shape)
    ...
    oname = f'feature1_ens0{ens}_{lon}E_{lat}N'
    ...
    oname = f'feature2_ens0{ens}_{lon}E_{lat}N'
    data.astype('float32').tofile(save_path+oname+'.gdat')
